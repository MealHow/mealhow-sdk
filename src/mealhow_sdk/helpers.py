...
async def to_snake_case(str_value: str) -> str:
    str_value = str_value.lower()
    str_value = re.sub(r"\W+", "_", str_value)
    return str_value.strip("_")


async def get_bmi(weight: int, height: int) -> float:
    return round((weight / (height**2)) * 10000, 1)


async def convert_weight_to_metric(weight: int) -> int:
    return int(round(0.453592 * weight))


async def convert_height_to_metric(height: int) -> int:
    return int(round(2.54 * height))


async def convert_weight_to_imperial(weight: int) -> int:
    return int(round(2.20462 * weight))


async def convert_height_to_imperial(height: int) -> int:
    return int(round(0.393701 * height))


async def get_basal_metabolic_rate_harris_benedict(weight: int, height: int, age: int, sex: BiologicalSex) -> int:
...
async def get_basal_metabolic_rate_mifflin_st_jeor(weight: int, height: int, age: int, sex: str) -> int:
    if sex == BiologicalSex.male.value:
...
async def get_calories_goal_by_activity_level(bmr_value: int, activity_level: str) -> int:
